...
```bash
video-translate-transcode$ sam build --use-container
```
...
```bash
video-translate-transcode$ sam logs -n StockCheckerFunction --stack-name video-translate-transcode --tail
```
...
```bash
video-translate-transcode$ pip install pytest pytest-mock --user
video-translate-transcode$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name video-translate-transcode
```
